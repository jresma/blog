...
(function() {
  ...
  Blog.Views.Post_Item = (function(_super) {
    var template;

    __extends(Post_Item, _super);

    function Post_Item() {
      this.render = __bind(this.render, this);
      return Post_Item.__super__.constructor.apply(this, arguments);
    }

    Post_Item.prototype.tagName = 'li';

    Post_Item.prototype.template = 'post-item';

    Post_Item.prototype.render = function() {};

    template = _h.template(Post_Item.template);

    Post_Item.$el.html(template);

    Post_Item.$el.find('#title').html(Post_Item.model.get('title'));

    Post_Item.$el.find('#permalink').html(Post_Item.model.get('permalink'));

    Post_Item.$el.find('#content').html(Post_Item.model.get('content'));
...
(function() {
  ...
  Blog.Views.Post_list = (function(_super) {
    ...
    function Post_list() {
      ...
      this.re_render = __bind(this.re_render, this);
    ...
    Post_list.prototype.addOne = function(model) {
      var item;
      item = new Blog.Views.Post_Item({
        model: model
      });
      this.$el.find('ul.post-list').append(item.render().el);
      return this;
    };

    Post_list.prototype.re_render = function(e) {
      this.render;
      return this;
    };
